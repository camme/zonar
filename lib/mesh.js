...
function Hitta(options) {
    ...
    var id = options.id.replace(/[\s]/g, '-').toLowerCase();
    ...
    function parsePayload(payload, senderInfo) {
        ...
        if (messageParts.length > 0 && messageParts[0] == broadcastIdentifier) {
            ...
            messageObject = {
                uniqueId: messageParts[1],
                id: messageParts[2],
                port: messageParts[3],
                listenPort: messageParts[4],
                status: messageParts[5],
                payload: messageParts[6]
            };
